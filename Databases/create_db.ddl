...
CREATE TABLE coop_nodes (
    ...
    cooperative_node_id NUMBER NOT NULL,
    address_id          INTEGER NOT NULL
);
...
ALTER TABLE coop_nodes ADD CONSTRAINT cooperative_node_pk UNIQUE ( cooperative_node_id );
...
CREATE TABLE nodes_packs_history (
    ...
    coop_nodes_node_id INTEGER NOT NULL
);
...
ALTER TABLE employees
    ADD CONSTRAINT employees_coop_nodes_fk FOREIGN KEY ( node_id )
        REFERENCES coop_nodes ( cooperative_node_id );
...
ALTER TABLE nodes_packs_history
    ADD CONSTRAINT nodes_packs_coop_node_fk FOREIGN KEY ( coop_node_id )
        REFERENCES coop_nodes ( cooperative_node_id );

ALTER TABLE nodes_packs_history
    ADD CONSTRAINT nodes_packs_fk FOREIGN KEY ( coop_nodes_node_id )
...
CREATE SEQUENCE coop_nodes_cooperative_node_id START WITH 1 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER coop_nodes_cooperative_node_id BEFORE
    INSERT ON coop_nodes
    FOR EACH ROW
    WHEN ( new.cooperative_node_id IS NULL )
BEGIN
    :new.cooperative_node_id := coop_nodes_cooperative_node_id.nextval;
END;
/
